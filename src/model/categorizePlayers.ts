...
function categorizePlayer(
    player: Partial<DetailedPlayer>,
    allPlayers: DetailedPlayer[]
): number {
...
function findAlternativePlayers(
...
async function analyzeTeam() {
    try {
        ...
        return players.map((player) => {
            ...
            const alternatives =
                player.tp && player.ap && player.mv
                    ? findAlternativePlayers(
                          player as Required<DetailedPlayer>,
                          players
                      )
                    : []
